...
def get_all_traites():
    ...
    # -------- 1. Clients traites
    total_clients = Traite.objects.aggregate(total=Sum("montant"))["total"] or 0
    ...
    echues_fournisseurs = TraiteFournisseur.objects.filter(date_echeance__lt=today, status="NON_PAYEE").aggregate(
        total=Sum("montant"))["total"] or 0
